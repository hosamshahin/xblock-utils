...
# License: AGPLv3
"""
This module contains a mixin that allows third party XBlocks to be easily
edited within edX Studio just like the built-in modules. No configuration
required, just add StudioEditableXBlockMixin to your XBlock.
"""
...
from xblock.fields import Scope, JSONField, List, Integer, Float, \
    Boolean, String
...
class FutureFields(object):
    ...
    """
    A helper class whose attribute values come from the specified dictionary
    or fallback object. This is only used by StudioEditableXBlockMixin and is
    not meant to be re-used anywhere else! This class wraps an XBlock and
    makes it appear that some of the block's field values have been changed to
    new values or deleted (and reset to default values). It does so without
    actually modifying the XBlock. The only reason we need this is because the
    XBlock validation API is built around attribute access, but often we want
    to validate data that's stored in a dictionary before making changes to an
    XBlock's attributes (since any changes made to the XBlock may get
    persisted even if validation fails).
    """

    def __init__(self, new_fields_dict, newly_removed_fields, fallback_obj):
        """
        Create an instance whose attributes come from new_fields_dict and
        fallback_obj.
        Arguments: new_fields_dict -- A dictionary of values
        that will appear as attributes of this object
        newly_removed_fields --
        A list of field names for which we will not use fallback_obj
        fallback_obj -- An XBlock to use as a provider for any attributes not
        in new_fields_dict
        """
    ...
    def __getattr__(self, name):
        try:
            ...
            if name in self._blacklist:
                # Pretend like this field is not actually set, since we're
                # going to be resetting it to default
...
class StudioEditableXBlockMixin(object):
    ...
    def _make_field_info(self, field_name, field):
        """
        Create the information that the template needs to render a form field
        for this field.
        """
        supported_field_types = (
            ...
            (String, 'string'),
            (List, 'list'),
            # This is last so as a last resort we display a text field w/ the
            # JSON string
            (JSONField, 'generic'),
        )
        info = {
        ...
        for type_class, type_name in supported_field_types:
            if isinstance(field, type_class):
                info['type'] = type_name
                # If String fields are declared like String(...,
                # multiline_editor=True), then call them "text" type:
                ...
                if type_class is List and field.runtime_options.get('list_style') == "set":
                    # List represents unordered, unique items, optionally drawn
                    # from list_values_provider()
                    info['type'] = 'set'
                elif type_class is List:
                    # disable other types of list for now until properly
                    # implemented
                    info['type'] = "generic"
                break
        if "type" not in info:
            raise NotImplementedError(
                "StudioEditableXBlockMixin currently only supports fields derived from JSONField")
        if info["type"] in ("list", "set"):
            ...
            # Convert value to JSON string if we're treating this field
            # generically:
            ...
            if isinstance(field.values, dict) and isinstance(field, (Float, Integer)):
                ...
                for option in field.values:
                    if option in ("min", "max", "step"):
                        ...
                        raise KeyError(
                            "Invalid 'values' key. Should be like values={'min': 1, 'max': 10, 'step': 1}")
                ...
                info['values'] = [
                    {"display_name": unicode(val), "value": val} for val in values]
        ...
        if info["type"] in ("list", "set") and field.runtime_options.get('list_values_provider'):
            ...
            # Furthermore, we need to convert all values to JSON since they
            ...
            if list_values and isinstance(list_values[0], dict) and "display_name" in list_values[0]:
                ...
                list_values = [
                    {"display_name": unicode(val), "value": val} for val in list_values]
    ...
    def submit_studio_edits(self, data, suffix=''):
        ...
        # make. So we create a "fake" object that has all the field values we
        # are about to set.
...
class StudioContainerXBlockMixin(object):
    ...
    def render_children(self, context, fragment, can_reorder=True, can_add=False):
        ...
        for child_id in self.children:
            ...
            rendered_child = child.render(
                'author_view' if hasattr(child, 'author_view') else 'student_view', context)
    ...
    def author_view(self, context):
        ...
        if root_xblock and root_xblock.location == self.location:
            # User has clicked the "View" link. Show an editable preview of
    ...
    def author_edit_view(self, context):
        ...
        self.render_children(
            context, fragment, can_reorder=True, can_add=False)
        return fragment
